...
/**
 * Compute the UTC send_at timestamp for a scheduled message.
 *
 * All offset calculations are done in Jerusalem local space first,
 * then converted to UTC — this is the correct DST-safe approach.
 * (Never subtract days in UTC and then format as local.)
 *
 * @param eventDateISO  - Event date in YYYY-MM-DD format
 * @param eventTimeHHMM - Event time in HH:MM format (Jerusalem wall clock)
 * @param offset        - One of the 4 offset labels
